...
const Page = () => {
    ...
    useEffect(() => {
        const getTotalPages = async () => {
            const totalQuery = query(collectionGroup(db, collection));
    ...
    return (
        <main className="">
            <SectionWrapper>
                <h2 className="uppercase font-bold font-headers bg-black text-white py-2 px-2">RECENT POEMS</h2>
                ...
                {loading ? <div className="h-[50vh] flex justify-center items-center">
                    ...
                    <div className="pt-4">
                        <p className="text-sm my-2 font-headers font-semibold">
                            Showing {startIndex} to {endIndex} of {totalArticles} poems
                        </p>
                        <div className="pt-14 grid sm:grid-cols-3 md:grid-cols-3 gap-5 md:gap-y-14 ">
                            {articles.map(poem => (
                                <PoemLink key={poem.id} poem={poem} />
                            ))}
                        </div>
