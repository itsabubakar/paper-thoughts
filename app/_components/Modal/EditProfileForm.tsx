...
const EditProfileForm: React.FC<EditProfileFormProps> = ({ closeModal, profile }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (user) {
            ...
            if (formState.penName) newDetails.penName = formState.penName;
            if (formState.about) newDetails.about = formState.about;
            if (formState.instagram) newDetails.instagram = formState.instagram;
            if (formState.twitter) newDetails.twitter = formState.twitter;
            if (formState.email) newDetails.email = formState.email;

            try {
                await setDoc(postRef, newDetails, { merge: true }); // merge true will update the data without overwriting existing fields

                console.log('Profile updated successfully');
                setProfile(newDetails);
                setLoading(false); // Update the profile context with the new details
                closeModal();
            } catch (error) {
                setLoading(false);
    ...
    return (
        <form className=" min-w-[340px] md:min-w-[520px]  rounded-lg " onSubmit={handleSubmit}>
